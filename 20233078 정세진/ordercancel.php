...
if (isset($_GET['confirm']) && $_GET['confirm'] === 'yes') {
    if(empty($randomnum)) {
        ...
        $deleteorderlist = mysqli_query($con, "DELETE FROM orderlist WHERE userid='$userid' AND session='$session' AND randomnum='$randomnum'");
        $deleteorderlist = mysqli_query($con, "DELETE FROM receivers WHERE userid='$userid' AND session='$session' AND randomnum='$randomnum'");
    }
    if ($deleteorderlist) {
        echo "<script>
            alert('주문이 취소되었습니다.');
