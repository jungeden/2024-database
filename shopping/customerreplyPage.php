...
$id = $_GET['id'];
$getparent=mysqli_query($con,"SELECT * FROM customerboard WHERE id='$id'");
$row=mysqli_fetch_assoc($getparent);
$topic = $row['topic'];
$content = $row['content'];
...
<body>
    <div class='container'>
        ...
        <div class='middle'>
            <form class='middle' method='post' action='customerreply.php?parentid=$id&issecret=$issecret&class=$class' enctype=\"multipart/form-data\">

                <div class='qnatextbox' style='margin:100px 0 0 0;'>
                    <div style='color:rgb(255,197,90); font-size:25px; margin-left:5px;'>관리자</div>
                    <input class='input' type='text' name='topic' placeholder='제목' value='$topic'>
                        <div class='classoptionbox'>
                            <button class='input' type='button' name='classoption' id='classoption' ><a style='color:rgb(28,28,28); '>$class</a></button>
                        </div>
                    <div id='editor'>
                    <pre>
                    \n
                 ...
                 <input class='button re' type='submit' value='등록'>
