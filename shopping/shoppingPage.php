...
if ($total == 0) {
    ...
    // 이전 블록 링크
    if ($pblock > 0) {
        echo "<a href='shoppingPage.php?cblock=$pblock&cpage=$pStartPage&userid=$userid'>
    ...
    for ($i = $startPage; $i < $nStartPage && $i <= $totalPage; $i++) {
        ...
        echo "<div class='pagenum'><a href='shoppingPage.php?cblock=$cblock&cpage=$i' $class>$i</a></div>";
    }

    // 다음 블록 링크
    if ($nStartPage <= $totalPage) {
        echo "<a href='shoppingPage.php?cblock=$nblock&cpage=$nStartPage&userid=$userid'>
            <svg xmlns='http://www.w3.org/2000/svg' height='24px' viewBox='0 -960 960 960' width='24px' fill='#181818'>
