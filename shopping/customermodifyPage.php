        ...
        <div class='middle'>
            <form class='middle' method='post' action='customermodify.php?id=$id' enctype=\"multipart/form-data\">
                    ...
                    <input class='input' type='hidden' name='class' id='class'>
                        <div class='classoptionbox'>
                            ...
                            <ul class='classinputoption hide' id='classselect'>
                                ...
                                <li><button type='button' class='input coption' id='기타' onclick='changeclass(this.id)' >기타</button></li>
                            </ul>
                        </div>
                ...
                <div class='productphoto de'>
                    <a class='phototext' >
                        <svg xmlns='http://www.w3.org/2000/svg' height='30px' viewBox='0 -960 960 960' width='30px' fill='rgb(255,197,90)'>
                            <path id='input' d='M460-460H240v-40h220v-220h40v220h220v40H500v220h-40v-220Z'/>
                        </svg>
                    </a>
                    <a class='phototext'  style='margin-top:35px;'>사진추가</a>
                    <input class='file' type='file' name=\"customerboardfile[]\" id='customerboardfile'  multiple accept='image/*'   onchange='previewImage(event, false)'>

                    <div class='preview-container'>
                        <img class='photo' src='./customerfiles/$customerfile'>
                    </div> <!-- 다중 이미지 미리보기를 위한 컨테이너 -->
                </div>
...
function toggleFixed(element, categories) {
    ...
    if (isFixed) {
        ...
        categoryElement.value = 'n'; 
    } else {
        element.classList.add('fixed'); // 'fixed' 클래스 추가
        categoryElement.value = element.id; 
    }
...
function changeclass(value) {
    ...
    const test = document.getElementById('class').value 
    console.log(test);
}
