...
<script>
function previewImage(event, isFirstInput) {
    ...
    if (isFirstInput) {
        previewContainer.innerHTML = ''; // 첫 번째 input에서 기존 이미지 초기화
    }

    const files = input.files;

    if (files && files.length > 0) {
        console.log('선택한 파일 수:', files.length);
...
function toggleFixed(element, categories) {
    ...
    const size = categoryElement.value || ''; 
    const isFixed = element.classList.contains('fixed'); 

    if (isFixed) {
        element.classList.remove('fixed'); 
        let array = size.split(',').filter((value) => value !== element.id); 
        categoryElement.value = array.join(','); 
    } else {
        element.classList.add('fixed'); // 'fixed' 클래스 추가
        categoryElement.value = size ? size + ',' + element.id : element.id; 
    }
    console.log(categories,':', categoryElement.value )
}
...
<body>
    <div class='container'>
        ...
        <div class='middle manageproducts'>
            <form class='form manageproducts' method='post' action='manageinput.php' enctype=\"multipart/form-data\">
                <div class='left middle'>
                    <div class='productphoto'>
                        <a class='phototext' >
                            <svg xmlns='http://www.w3.org/2000/svg' height='30px' viewBox='0 -960 960 960' width='30px' fill='rgb(225,218,201)'>
                                <path id='input' d='M460-460H240v-40h220v-220h40v220h220v40H500v220h-40v-220Z'/>
                            </svg>
                        </a>
                        <a class='phototext' style='margin-top:35px;'>사진추가</a>
                        <input class='file' type='file' name='userfile' accept='image/*' onchange='previewImage(event, true)'>
                        <div class='preview-container'></div> <!-- 개별 이미지 미리보기를 위한 컨테이너 -->
                    </div>
                    ...
                    <div class='productphoto de'>
                        ...
                        <a class='phototext'  style='margin-top:35px;'>사진추가</a>
                        <input class='file' type='file' name=\"detailuserfile[]\" id='detailuserfile'  multiple accept='image/*'   onchange='previewImage(event, false)'>

                        <div class='preview-container'></div> <!-- 다중 이미지 미리보기를 위한 컨테이너 -->
                    </div>");
                    $getproductcode = mysqli_query($con, "SELECT MAX(code) AS last_code FROM product");
                    $prow=mysqli_fetch_assoc($getproductcode);
                    $lastcode = $prow['last_code'] ?? 0; 
                    $nextcode = $lastcode + 1; 
                    $formatcode = sprintf("%03d", $nextcode); 
...
echo("
                ...
                <div class='center middle'>
                    <div class='productinfo'>
                        <input class='input' type='text' name='code' placeholder='상품코드' value='$formatcode'>
                        <input class='input' type='hidden' name='class' id='class'  placeholder='상품분류'>
                        <div class='classoptionbox'>
                            <button class='input' type='button' name='classoption' id='classoption' onclick='showOption(\"class\")'><a style='color:rgb(225,218,201); font-size:13px;'>상품분류</a></button>
                        ...
                        <div class='optionscheck'>
                            ...
                            <div class='input options'>
                                <div class='filteroption'>
                                    <div class='check' id='xs' onclick=\"toggleFixed(this,'size')\"></div>
                                    <a class='ho' >xs</a>
                                </div>
                                <div class='filteroption'>
                                    <div class='check' id='s' onclick=\"toggleFixed(this,'size')\"></div>
                                    <a class='ho'>s</a>
                                </div>
                                <div class='filteroption'>
                                    <div class='check' id='m' onclick=\"toggleFixed(this,'size')\"></div>
                                    <a class='ho'>m</a>
                                </div>
                                <div class='filteroption'>
                                    <div class='check' id='l' onclick=\"toggleFixed(this,'size')\"></div>
                                    <a class='ho'>l</a>
                                </div>
                                <div class='filteroption'>
                                    <div class='check' id='xl' onclick=\"toggleFixed(this,'size')\"></div>
                                    <a class='ho'>xl</a>
                                </div>
                            </div>
                            <input class='input options' id='color' type='text' name='color' style='display:none;'>
                            <div class='input options' style='width:80px;'>
                                ...
                                <div class='filteroption'>
                                    <div class='check' id='black' onclick=\"toggleFixed(this,'color')\"></div>
                                    <a class='ho'>black</a>
                                </div>
                                <div class='filteroption'>
                                    <div class='check' id='gray' onclick=\"toggleFixed(this,'color')\"></div>
                                    <a class='ho'>gray</a>
                                </div>
                                <div class='filteroption'>
...
let fileInput = document.querySelector('#detailuserfile');
