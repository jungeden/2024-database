...
<div class='line'></div>");
    ...
    if (!empty($comments)) {
        ...
        $ocommentname=$_GET['commentname'];
        $ocommentmessage=$_GET['commentmessage'];
        $odate=$_GET['date'];

        foreach ($comments as $pcomment) {
            if($ocommentname==$pcomment['commentname']&&$ocommentmessage==$pcomment['commentmessage']&&$odate=$pcomment['date']) {
                continue;
            }
            echo ("
            ...
            <tr>
                <td style='vertical-align: bottom;' colspan=2>
                    <div class='commentname'>
                        ...
                        <a href='commentdelete.php&board=$board?commentname={$pcomment['commentname']}&date={$pcomment['date']}&commentmessage={$pcomment['commentmessage']}&id=$id&board=$board' style='position:absolute; right:5px;'>
    ...
    // 댓글 입력 폼 출력
    echo ("
    <form method='post' action='commentmodify.php?board=$board&id=$sid&date=$odate&commentname=$ocommentname'>
    ...
    <table style='border-spacing: 0; margin: 0;'>
        <tr>
            <td align='right' style='padding: 0; width:auto;'> <div class='commentname comment' ><a  name='commentwriter'>$ocommentname</a> 
            <a style='font-size:12px; color:gray; a:hover {color:gray}; margin-left:5px;'>({$pcomment['date']})</a></div></td>
            
        </tr>
        <tr>
            <td colspan=2 align='right' style='padding: 0; width:auto;'><textarea class='commentinput comment' style='width=500px; background-color:rgb(56,56,56);'  name='commentmessage' rows='4' cols='84'>$ocommentmessage</textarea></td>
        </tr>
        </table>
    </td>
    <td style='padding: 0;  align='left'>
        <table style='border-spacing: 0; margin: 0; '>
            <td align='left' width=10> <input class='commentbutton comment' type=submit value=수정 ></td>
        </table>
    </td>
    <tr>
    </table>
